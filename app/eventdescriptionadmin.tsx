...
import CustomHeader from '../components/CustomHeaderAdmin';
...
interface ApiTicketType {
  ...
  name: string;
  price: number;
  quantity: number; // Assumindo que esta coluna exista na sua tabela event_ticket_types
}
...
export default function EventDescriptionAdminScreen() {
  ...
  const processAndCalculateStats = (ticketTypes: ApiTicketType[], soldTickets: ApiSoldTicket[]) => {
    ...
    const processedDetails: ProcessedTicketDetails[] = ticketTypes.map((type) => {
      ...
      return {
        ...
        available_quantity: (type.quantity || 0) - sold_quantity,
    ...
    const grandTotals = processedDetails.reduce(
      (acc, ticket) => {
  ...
  const formatDate = (dateString: string) =>
    new Date(dateString).toLocaleString('pt-BR', { dateStyle: 'full', timeStyle: 'short' });
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.eventContentWrapper}>
            ...
            <ThemedText style={styles.eventDateLocation}>
            ...
            <View style={styles.buttonRow}>
              ...
              >
                <ThemedText style={styles.buttonText}>
                  {showAdminDetails ? 'Ocultar Detalhes' : 'Ver Detalhes Admin'}
                </ThemedText>
              </TouchableOpacity>
            </View>
              ...
              <>
                ...
                <View style={styles.adminCardsContainer}>
                  <View style={styles.adminCard}>
                    ...
                    <ThemedText style={styles.adminCardValue}>{totals.totalAvailable}</ThemedText>
                  </View>
                  <View style={styles.adminCard}>
                    <ThemedText style={styles.adminCardTitle}>Vendidos</ThemedText>
                    <ThemedText style={styles.adminCardValue}>{totals.totalSold}</ThemedText>
                  </View>
                  <View style={styles.adminCard}>
                    <ThemedText style={styles.adminCardTitle}>Presentes</ThemedText>
                    <ThemedText style={styles.adminCardValue}>{totals.attendees}</ThemedText>
                  </View>
                </View>
                ...
                {processedTickets.map((ticket) => (
                  <View key={ticket.id} style={styles.ticketDetailCard}>
                    ...
                    <ThemedText style={styles.ticketDetailInfo}>
                      {ticket.sold_quantity} / {ticket.quantity || 'N/A'} vendidos (
                      {formatCurrency(ticket.price)})
                    </ThemedText>
                    <ThemedText style={styles.ticketDetailRevenue}>
                      Receita: {formatCurrency(ticket.revenue)}
                    </ThemedText>
                  </View>
                ))}
